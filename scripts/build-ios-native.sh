...
if [[ -n "${IOS_ONNX_RUNTIME_DIR}" ]]; then
  ...
  CARGO_FEATURES+=(ios-dual-backend)
...
# whisper.cpp's CMake build reads Apple's minimum deployment target from this
# variable for both device and simulator toolchains.
export MACOSX_DEPLOYMENT_TARGET="${IPHONEOS_DEPLOYMENT_TARGET:-16.0}"
export CMAKE_OSX_DEPLOYMENT_TARGET="${IPHONEOS_DEPLOYMENT_TARGET:-16.0}"
...
build_target() {
  ...
  local cargo_env=(
    ...
    "MACOSX_DEPLOYMENT_TARGET=${IPHONEOS_DEPLOYMENT_TARGET:-16.0}"
    "CMAKE_OSX_DEPLOYMENT_TARGET=${IPHONEOS_DEPLOYMENT_TARGET:-16.0}"
  )
  ...
  if [[ -n "${IOS_ONNX_RUNTIME_XCFRAMEWORK}" ]]; then
    ...
    cargo_env+=(ORT_LIB_LOCATION="${link_dir}" ORT_SKIP_DOWNLOAD=1 ORT_PREFER_DYNAMIC_LINK=0)
  fi
